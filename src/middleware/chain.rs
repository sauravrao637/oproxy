...
impl MiddlewareChain {
    ...
    /// Builds the protocol-aware execution plan from the body hints declared by
    /// the active plugins (see [`Middleware::body_hint`]). Computed from the
    ...
    /// buffered class; see [`crate::core::forward::plan_execution`] for the rules.
    ...
    pub async fn execute_response(&self, ctx: &mut ResponseContext) -> MiddlewareAction {
        // Reverse order preserves stack semantics around the upstream request.
