...
pub fn management_router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route("/admin/rewrites/:index", axum::routing::delete(delete_rewrite))
...
async fn reload_config(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.proxy_engine.set_max_body_bytes(new_cfg.max_body_bytes);
    tracing::info!(max_body_bytes = new_cfg.max_body_bytes, "Config hot-reloaded");
    axum::Json(serde_json::json!({
        "reloaded": true,
        "max_body_bytes": new_cfg.max_body_bytes,
